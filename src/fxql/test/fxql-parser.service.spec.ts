...
describe('FXQLParserService', () => {
  ...
  describe('parse', () => {
    it('should throw error for lowercase currency', () => {
      expect(() => 
        service.parseMultiple('usd-GBP { BUY 100 SELL 200 CAP 93800 }')
    ...
    it('should throw error for invalid numeric amount', () => {
      expect(() => 
        service.parseMultiple('USD-GBP { BUY abc SELL 200 CAP 93800 }')
      ).toThrow(BadRequestException);
    });

    it('should throw error for negative CAP', () => {
      expect(() => 
        service.parseMultiple('USD-GBP { BUY 100 SELL 200 CAP -50 }')
      ).toThrow(BadRequestException);
    });

    it('should throw error for empty statement', () => {
      expect(() => 
        service.parseMultiple('USD-GBP { }')
      ).toThrow(BadRequestException);
    });

    it('should throw error for multiple statements', () => {
      expect(() => 
        service.parseMultiple('USD-GBP { BUY 100 SELL 200 CAP 93800 } EUR-JPY { BUY 80 SELL 90 CAP 50000 }')
      ).toThrow(BadRequestException);
    });

    it('should parse valid statement', () => {
      const result = service.parseMultiple('USD-GBP { BUY 100 SELL 200 CAP 93800 }');
